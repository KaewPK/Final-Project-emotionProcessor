...
##  found in the generic user profile.
    def generateProfile(self):
        ...
        generic = pandas.read_csv("profiles/"+self.model, header = None, sep = ',', names = ['Pitch', 'Tone', 'SPL', 'wordGap' , 'WordGapLen', 'Emotion', 'Score'])
        # Write the dataframe to a new .csv file that will be associated with the
        #   current user profile
        generic.to_csv(self.path, index = False, header = 0)
        return self.path
		
##	Method that allows for data to be appended to a user profile while not overwriting previous data
